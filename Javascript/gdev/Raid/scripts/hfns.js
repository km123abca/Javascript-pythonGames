function get_stuff_from_parent_url(pivot_word = "=", pos = 0) {
  ...
  if (extracted_variable == -1) return "";
  return extracted_variable;
}

function y_seperated(obj1, obj2) {
  if (obj1.posy + obj1.height / 2 < obj2.posy - obj2.height / 2) return true;
  if (obj1.posy - obj1.height / 2 > obj2.posy + obj2.height / 2) return true;
  return false;
}
function x_seperated(obj1, obj2) {
  if (obj1.posx + obj1.width / 2 < obj2.posx - obj2.width / 2) return true;
  if (obj1.posx - obj1.width / 2 > obj2.posx + obj2.width / 2) return true;
  return false;
}

function boxcaster(targetobj, sourceobj2, rayLength, angleRange, scan = false) {
  //sourceobj => {posx:33,posy:44,width:20,height:20}  angleRange=>[90,91]

  var sourceobj = {
    posx: sourceobj2.posx,
    posy: sourceobj2.posy,
    width: sourceobj2.width,
    height: sourceobj2.height,
  };
  var start_angle = 0,
    start_length;
  var end_angle = 0;
  ...
  for (; start_angle < end_angle; start_angle++) {
    ...
    while (start_length < rayLength) {
      ...
      sourceobj.posy =
        sourceobj2.posy +
        start_length * Math.sin((start_angle / 180) * Math.PI);
      sourceobj.posx =
        sourceobj2.posx +
        start_length * Math.cos((start_angle / 180) * Math.PI);
      ...
      if (scan) {
        drawLine(
          sourceobj2.posx - camx,
          sourceobj2.posy - camy,
          sourceobj.posx - camx,
          sourceobj.posy - camy
        );
      }
      if (collisioncheckz(sourceobj, targetobj)) {
        // drawLine(
        //   sourceobj2.posx,
        //   sourceobj2.posy,
        //   sourceobj.posx,
