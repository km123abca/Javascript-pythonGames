...
BULLETIMAGE=pygame.image.load('./sprites/redSphere.png')
BULLETSIZEFAC=0.05
...
class bullet:
	def __init__(self,x,y,angle,parent,bulletType):
		self.onScreen=True
...
class beetle:
	def __init__(self,x,y,gateIndex):
		...
		self.hornCollider=collisionBox(x,y,self.width*0.8,self.height*0.8)
		self.position=CreateVector(x,y)
		...
		self.enemyProximity=200*v.xscale
		...
		self.plungeVelocity=CreateVector(self.velMax*2,0)
		self.focussed=False
	...
	def shoot(self):
		if time.time()-self.lastShootTime > self.shootGap:						
			self.bullets.append(bullet(self.position.x,self.position.y,self.angle,self,'enemy'))				
			v.shootingSound.play()		
