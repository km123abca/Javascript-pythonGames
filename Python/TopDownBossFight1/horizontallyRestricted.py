...
class Player:
	...
	def update(self):
		...
		if self.staggering:
			...
			if LEFTPRESSED:
				self.adjustToDestinationAngle(180)
				# self.IncreaseRotationSpeed()
				self.ChangeAnimation("move")
				# self.ApplyVelocity()
			elif RIGHTPRESSED:
				self.adjustToDestinationAngle(0)
				# self.IncreaseRotationSpeed()
				self.ChangeAnimation("move")
				# self.ApplyVelocity()
			elif UPPRESSED:
				self.adjustToDestinationAngle(270)
				# self.IncreaseRotationSpeed()
				self.ChangeAnimation("move")
				# self.ApplyVelocity()
			elif DOWNPRESSED:
				self.adjustToDestinationAngle(90)
				# self.IncreaseRotationSpeed()
				self.ChangeAnimation("move")
				# self.ApplyVelocity()
			else:
	...
	def display(self):
		...
		myrect.centerx,myrect.centery=self.position.x-self.width/2-gameManager.camera.position.x,self.position.y-self.height/2-gameManager.camera.position.y
