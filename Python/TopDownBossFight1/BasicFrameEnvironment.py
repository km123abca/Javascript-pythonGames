...
class collisionBox:
	...
	def updateP(self,pos):
		if not self.position.equals(pos):
			self.position.setVec(pos)
################## Collision Box Ends ######################################
...
class BrickWall:
	def __init__(self,x,y,width,height,hori=True):
		...
		if hori:
			self.leftCollider=  positionBlock(x,y,-0.495*width,0,0,int(0.01*width),int(0.8*height),(0,0,255))
			self.rightCollider= positionBlock(x,y,0.495*width,0,0,int(0.01*width),int(0.8*height),(0,0,255))
			self.topCollider=   positionBlock(x,y,0,-0.45*height,0,width,int(0.1*height))
			self.bottomCollider=positionBlock(x,y,0,0.45*height,0,width,int(0.1*height))
		else:
			self.leftCollider=  positionBlock(x,y,-0.48*width,0,0,int(0.04*width),height,(0,0,255))
			self.rightCollider= positionBlock(x,y,0.48*width,0,0,int(0.04*width),height,(0,0,255))
			self.topCollider=   positionBlock(x,y,0,-0.495*height,0,int(0.99*width),int(0.01*height))
			self.bottomCollider=positionBlock(x,y,0,0.495*height,0,int(0.99*width),int(0.01*height))
...
class Player:
	...
	def RotateTowardsTarget(self):
		targetAngle=self.player_s_target.position.copy().sub(self.position).heading()
		self.angle=Lerp(self.angle,targetAngle,4)
	def RotateAwayFromTarget(self):
		targetAngle=self.position.copy().sub(self.player_s_target.position).heading()		
		self.angle=Lerp(self.angle,targetAngle,4)
	...
	def LockedMovement(self):
		if OPRESSED and not self.OTRIGGER:
			...
			if self.OTRIGGER:
				self.OTRIGGER=False
		if not self.runningBack:
			self.RotateTowardsTarget()
		else: 
			self.RotateAwayFromTarget()
	...
	def FixVelocityOnLock(self):
		...
		#There was a velocity assignment here initially which was removed
	...
	def ApplyVelocity(self,runBack=False):
		if self.dashOn:
	...
	def DeApplyVelocity(self,runBack=False):
		if self.vel <=self.velMin:
...
def SpawnCloudGroup(pos):
	for i in range(30):
		...
		gameManager.volatile_gos.append(gameManager.cloudsList[-1])
...
class GameManager:
	def __init__(self):
		...
		self.allGameObjects,self.camStoppers=[],[]
		self.roadBlocksList,self.cloudsList,self.wallsList,self.enemiesList,self.camStoppers,self.nsobjs,self.gatesList=[],[],[],[],[],[],[]
		self.rocks=[]
		self.non_volatile_gos,self.volatile_gos=[],[]
		self.player=None
	...
	def loadScene(self,i):
		...
		filex=open('./SampleSceneData.txt','r')
	...
	def LoadObjects(self,line):
		...
		if not re.search(r'[0-9]',line):
			...
			if self.loadingType=='wall':
				...
				self.non_volatile_gos.append(self.wallsList[-1])		
				...
				self.non_volatile_gos.append(self.gatesList[-1])		
				...
				self.zombiesList.append(Zombie(int(nums[0]),int(nums[1]),int(nums[2]),int(nums[3])))
				self.volatile_gos.append(self.zombiesList[-1])
				...
				self.non_volatile_gos.append(self.camStoppers[-1])				
				...
				self.non_volatile_gos.append(self.nsobjs[-1])
	...
	def unloadComponents(self):	
		self.player=None	
		self.roadBlocksList,self.cloudsList,self.wallsList,self.enemiesList,self.camStoppers,self.nsobjs=[],[],[],[],[],[]
		self.allGameObjects,self.volatile_gos,self.non_volatile_gos=[],[],[]
	...
	def runGame(self):	
		...
		if self.player and self.player.onScreen:		
			self.player.update()
			self.player.display()
		for x in self.volatile_gos:
			if x.onScreen:
				x.update()
				x.display()
		flushList(self.volatile_gos)	
		for x in self.non_volatile_gos:
			if x.onScreen:
				x.update()
				x.display()
'''
