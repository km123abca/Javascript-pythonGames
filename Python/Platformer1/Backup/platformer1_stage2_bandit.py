...
class GameManager:
	...
	def InformAboutGate(self,bossID,btrigger):
		...
		if bossID==0 and self.currentScene==0:
			...
			self.roadBlocksList.append(fightGatesList[-1])
			self.banditsList.append(Bandit(3070*xscale,522*yscale,300,300,fightGatesList[-1],fightGatesList[-2],btrigger))
	...
	def LoadObjects(self,line):
		...
		if not re.search(r'[0-9]',line):
			...
			if self.loadingType=='pad':
				...
				# pChanger=posChanger(padsList[-1])	
	...
	def unloadComponents(self):
		self.platformList,self.movingPlatforms,self.padsList=[],[],[]		
		triggersList,spheresList,fightGatesList,enemiesList,cloudsList,padsList,self.collidableObjects,self.roadBlocksList=[],[],[],[],[],[],[],[]
